...
class AstrologyFacts:
    ...
    def get_facts(self):
        zodiac_fact = f"\nZODIAC SIGN (Sun Sign): {self.user.get_zodiac_sign()} \n{self.get_zodiac_sign_description()}"
        moon_fact = f"\nMOON SIGN: {self.user.get_moon_sign()} \n{self.get_moon_sign_description()}"
        rising_fact = f"\nRISING SIGN: {self.user.calculate_rising_sign()} \n{self.get_rising_description()}"
        chinese_fact = f"\nCHINESE ZODIAC SIGN: {self.user.calculate_chinese_zodiac()} \n{self.chinese()}"
...
class CompatibilityDialog(tk.Toplevel):
    def __init__(self, parent, user1_zodiac, user2_zodiac, percentage, description):
        ...
        tk.Label(self, text="Compatibility Result", font=("Arial", 18, "bold"), bg="#D7DDDF").pack(pady=10)
        tk.Label(self, text=f"Your Zodiac Sign: {user1_zodiac}", font=("Arial", 14), bg="#D7DDDF").pack()
        tk.Label(self, text=f"Their Zodiac Sign: {user2_zodiac}", font=("Arial", 14), bg="#D7DDDF").pack()
        tk.Label(self, text=description, wraplength=380, justify='center', font=("Arial", 12), bg="#D7DDDF").pack(pady=10)
        ...
        tk.Label(self, text=f"{percentage}%", font=("Arial", 36, "bold"), bg="#D7DDDF").pack(pady=10)
...
class AstraiOS:
    ...
    def val_ai_tarot_reading(self):
        ...
        if tarot_choice_dialog.choice == "Daily Draw":
            ...
            self.show_custom_dialog("Fortune Telling",
                                    ...
                                    f"Past: {fortune['past'][0]} ({fortune['past'][1]})\nDescription: {fortune['past'][2]}\n\n"
                                    f"Present: {fortune['present'][0]} ({fortune['present'][1]})\nDescription: {fortune['present'][2]}\n\n"
                                    f"Future: {fortune['future'][0]} ({fortune['future'][1]})\nDescription: {fortune['future'][2]}")
    ...
    def check_compatibility(self):
        ...
        if choice:
            ...
            selected_user = simpledialog.askstring("Select User", "Enter the name of the user you want to check compatibility with:")
        ...
        if compatibility_percentage <= 10:
            description = "Your compatibility is quite low. You might face significant challenges in this relationship. However, with patience and effort, it can still work out."
        elif compatibility_percentage <= 30:
            description = "Your compatibility is moderate. There are some obstacles, but with mutual understanding and communication, you may overcome them."
        elif compatibility_percentage <= 50:
            description = "Your compatibility is fair. It could go either way, depending on how much effort both parties are willing to put into the relationship."
        elif compatibility_percentage <= 70:
            description = "Your compatibility is good. You have much in common and share similar values, which can lead to a harmonious relationship."
        elif compatibility_percentage <= 90:
            description = "Your compatibility is great. You are highly compatible and share a deep connection. Your relationship is likely to be very fulfilling and rewarding."
        else:
            description = "Your compatibility is excellent. You are a perfect match and complement each other in every way. Your relationship is destined for success and happiness."
